...
def main():
    ...
    em = sde.euler_maruyama(x0, T=T, n_steps=n_steps, a=lambda x, t: (3-x)*t**2, b=lambda x, t: x**(.5))
    mil = sde.milstein(x0, T=T, n_steps=n_steps, a=lambda x, t: (3-x)*t**2, b=lambda x, t: x**(.5), db_dx=lambda x, t: 0.5*(x)**(-.5))
