...
from alt_implementations.raw_python_implementation import (
    euler_maruyama as py_euler_maruyama,
    milstein as py_milstein,
)
from sde_simulation import (
    euler_maruyama as cpp_euler_maruyama,
    milstein as cpp_milstein,
)
...
@pytest.mark.parametrize("x0, T, n_steps, a, b, dbdx", [
    # Case 1: Constant drift and diffusion (dbdx = 0)
    (1.0, 1.0, 100, lambda x, t: 0.1, lambda x, t: 0.2, lambda x, t: 0.0),
    # Case 2: Linear/GBM-like drift and diffusion
    (2.5, 3.0, 500, lambda x, t: 0.5 * x, lambda x, t: 0.3 * x, lambda x, t: 0.3),
    # Case 3: Non-linear time-dependent drift/diffusion
    (0.5, 2.0, 1000, lambda x, t: (2.0 - x) * t, lambda x, t: (1.0 + x**2) * (1.0 + t), lambda x, t: 2.0 * x * (1.0 + t)),
    # Case 4: Zero drift, constant diffusion
    (3.0, 5.0, 250, lambda x, t: 0, lambda x, t: 1, lambda x, t: 0),
])
def test_milstein_equivalence(x0, T, n_steps, a, b, dbdx):
    # Use a fixed seed for reproducible random increments
    seed = 0
    rng = np.random.default_rng(seed)
    ...
    # Get results from raw python alternative implementation
    path_py = py_milstein(x0, z, T=T, a=a, b=b, dbdx=dbdx)
    
    # Get results from main C++-backed module
    path_cpp = cpp_milstein(
        x0=x0,
        T=T,
        n_steps=n_steps,
        a=a,
